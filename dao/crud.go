...
// 根据GitId列表获取服务信息列表
func GetServiceListByGitId(gitIdList []string) (serviceList []Service, err error) {
	handle, err := cayley.NewGraph("bolt", config.Bolt_DB_Path, nil)
	if err != nil {
		return
	}
	...
	serviceList = make([]Service, 0)
    if len(gitIdList) == 0 {
        err = ServiceGitIdListIsNil
        return
    }
    it := path.StartPath(handle.QuadStore, gitIdList...).In(ServiceGitIdPre).Out(ServiceInfoPre).BuildIterator()
    for graph.Next(it) { // 循环获取所有的服务信息JSON
        serviceJson := handle.QuadStore.NameOf(it.Result())
        service := &Service{}
        err = ffjson.Unmarshal([]byte(serviceJson), service)
...
func GetServiceListEachLink(serviceNameList []string) (serviceLinkList []ServiceLink, err error) {
	serviceLinkList = make([]ServiceLink, 0)
	if len(serviceNameList) == 0 {
		err = ServiceNameListIsNil
		return
	...
	serviceLinkMap := make(map[string]string)
	for _, serviceName := range serviceNameList {
		serviceLinkList = getServiceLinkTargetInList(serviceName, serviceNameList, serviceLinkMap, serviceLinkList, handle.QuadStore)

		serviceLinkList = getServiceLinkSourceInList(serviceName, serviceNameList, serviceLinkMap, serviceLinkList, handle.QuadStore)
	}
...
func getServiceLinkTargetInList(serviceName string, serviceNameList []string,
	serviceLinkMap map[string]string, serviceLinkList []ServiceLink,
	qs graph.QuadStore) []ServiceLink {

	it := path.StartPath(qs, serviceName).Out(ServiceLinkPre).Is(serviceNameList...).BuildIterator()
	for graph.Next(it) {
		target := qs.NameOf(it.Result())
		serviceLinkStr := serviceName + "==>" + target
		if serviceLinkMap[serviceLinkStr] == "" {
			serviceLink := &ServiceLink{serviceName, target}
			serviceLinkList = append(serviceLinkList, *serviceLink)
			serviceLinkMap[serviceLinkStr] = serviceLinkStr
		}
	}
...
func getServiceLinkSourceInList(serviceName string, serviceNameList []string,
	serviceLinkMap map[string]string, serviceLinkList []ServiceLink,
	qs graph.QuadStore) []ServiceLink {

	it := path.StartPath(qs, serviceName).In(ServiceLinkPre).Is(serviceNameList...).BuildIterator()
	for graph.Next(it) {
		source := qs.NameOf(it.Result())
		serviceLinkStr := source + "==>" + serviceName
		if serviceLinkMap[serviceLinkStr] == "" {
			serviceLink := &ServiceLink{source, serviceName}
			serviceLinkList = append(serviceLinkList, *serviceLink)
			serviceLinkMap[serviceLinkStr] = serviceLinkStr
		}
	}
...
func convertServiceToQuads(service Service) (quads []quad.Quad, err error) {
	...
	// GitId
	if service.GitId != "" {
		quads = append(quads, cayley.Quad(service.Name, ServiceGitIdPre, service.GitId, ""))
	}
